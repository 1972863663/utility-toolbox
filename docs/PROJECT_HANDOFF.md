...
`Windows 实用工具箱` 是一个面向 Windows 的本地桌面工具集合，主程序使用 `Tkinter` 编写，通过 `PyInstaller` 打包成单文件可执行程序。它把多个日常 Windows 小工具整合在一个窗口里，包括批量重命名、窗口置顶、存档备份、视频字幕同步、音频输出切换、直播快捷入口、系统工具、开机自启动、拖拽关闭应用、照片视频整理、宏工具，以及一个独立的麦克风状态悬浮窗助手。
...
| `TrayIcon` | `utility_toolbox.py` | 系统托盘菜单、打开/隐藏工具箱、退出、置顶快捷项，并在 Explorer/任务栏重启后自动恢复图标。 |
...
| `MacroToolsTab` | `utility_toolbox.py` | 宏录制、循环执行及快捷键配置入口。 |
...
- “启动当前组”会先连续发出所有 `os.startfile` 请求，不再每启动一个应用固定等待 2 秒。
- “启动选中”如果只启动单个项目，会在后台延迟学习 `close_paths`；整组启动不把新进程路径乱写到每个项目，避免后续单独关闭时误杀同组程序。
- “关闭当前组”会先一次性匹配所有目标进程，再用单次批量 `taskkill /T /F /PID ...` 关闭；批量关闭失败时才回退到逐个关闭。
...
入口类：`TrayIcon`、`UtilityToolbox`

- 点击主窗口右上角关闭按钮只会执行 `withdraw()`，工具箱继续在通知区域运行。
- 真正退出必须使用托盘菜单的“退出”。
- Windows Explorer 或任务栏重启会清空第三方通知区域图标。托盘窗口监听系统广播
  `TaskbarCreated`，收到后重新调用 `Shell_NotifyIcon(NIM_ADD, ...)` 恢复图标。
- `test_launcher_close.py` 包含任务栏恢复回归测试；修改托盘生命周期后必须运行。
...
- GitHub 默认分支修复前基线为 `c7dc543 Integrate FPS overlay into utility toolbox (#1)`；本地已移除帧率监测集成及其可再部署资源。
- `README.md` 是简要说明；本文件是详细交接文档。
- 当前源码测试为 `18 passed`，PyInstaller 6.20.0 构建通过，桌面 `工具箱.exe` 已更新。
- 已实测：通知区图标被移除后发送 `TaskbarCreated` 可恢复；关闭主窗口后进程和托盘仍在；托盘点击可重新打开主窗口。
